...
ACTION MinSumGetAction(const Robot& robot, const AsteroidMap &a)
{
    ...
    float distance = Distance(nearest.GetPosition(), robot.GetPosition()) +
        Distance(nearest.GetPosition(), a.GetBases().front().GetPosition());
    for(const Resource &r : a.GetResources())
    {
        for(const Base &b : a.GetBases())
        {
            if(distance > Distance(r.GetPosition(), robot.GetPosition()) +
                Distance(r.GetPosition(), b.GetPosition()) && r.GetAmount() > 0)
            {
                nearest = r;
                distance = Distance(r.GetPosition(), robot.GetPosition()) +
                    Distance(r.GetPosition(), b.GetPosition());
            }
        }
    }
    // horizontal movement
